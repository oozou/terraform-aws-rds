locals {
  ...
  db_instance_address              = element(concat(aws_db_instance.this.*.address, [""]), 0)
  db_instance_arn                  = element(concat(aws_db_instance.this.*.arn, [""]), 0)
  db_instance_availability_zone    = element(concat(aws_db_instance.this.*.availability_zone, [""]), 0)
  db_instance_endpoint             = element(concat(aws_db_instance.this.*.endpoint, [""]), 0)
  db_instance_hosted_zone_id       = element(concat(aws_db_instance.this.*.hosted_zone_id, [""]), 0)
  ...
  db_instance_domain               = element(concat(aws_db_instance.this.*.domain, [""]), 0)
  db_instance_domain_iam_role_name = element(concat(aws_db_instance.this.*.domain_iam_role_name, [""]), 0)
  db_instance_master_password      = element(concat(aws_db_instance.this.*.password, [""]), 0)
}
...
output "secret_manager_rds_creds_arn" {
  value = try(aws_secretsmanager_secret.rds_creds[0].arn, "") #var.is_manage_master_user_password ? aws_db_instance.this[0].master_user_secret[0].secret_arn : try(aws_secretsmanager_secret.rds_creds[0].arn, "")
}
